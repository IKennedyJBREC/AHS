...
PredProps <- read.xlsx("C:/Users/ikennedy/JBREC/BP research public use microdata coding and data - General/AHSProject/OutsideDataSources_SingleYearApprox/AllCorrelations_1999_2009.xlsx", sheet = 2)
PredProps <- PredProps %>%
...
Template$MiniPredictor1 <- PredictorData$TotalPrivateConstructionSpendingManufacturingintheUnitedStates
Template$MiniPredictor2 <- PredictorData$`Expenditures_FloorCoveringsbyIncomeBeforeTaxes_$40_000to$49_999`
Template <- Template %>%
  left_join(Mini, by = 'Year')

Template$SmallPredictor1 <- PredictorData$ElectronicsandApplianceStores
Template$SmallPredictor2 <- PredictorData$`Expenditures_HouseholdFurnishingsandEquipmentbyQuintilesofIncomeBeforeTaxes_Fourth20Percent(61stto80thPercentile)`

Template <- Template %>%
  left_join(Small, by = 'Year')

Template$MediumPredictor1 <- PredictorData$Expenditures_MajorAppliancesbyHousingTenure_HomeOwner
Template <- Template %>%
  left_join(Medium, by = 'Year')

Template$LargePredictor1 <- PredictorData$`Expenditures_MajorAppliancesbyQuintilesofIncomeBeforeTaxes_Highest20Percent(81stto100thPercentile)`
Template$LargePredictor2 <- PredictorData$BuildingMaterialsandSuppliesDealers

MedianPrices <- read.xlsx("C:/Users/ikennedy/JBREC/BP research public use microdata coding and data - General/AHSProject/OutsideDataSources_SingleYearApprox/MedianHomePrices.xlsx")
Template$LargePredictor3 <- MedianPrices$MedianPrice
Template <- Template %>%
  left_join(Large, by = 'Year')

Template$PrecipPredictor1 <- PredictorData$May_CR_EastUS_RCP4.5
...
for (i in c(seq(1994,2021,1))){
  ...
  if (i %% 2 == 0){
    ...
    Template[Template$Year == i, ]$Large <-
      ...
      ((Template[Template$Year == i+1, ]$TotalJobCost_Large * (Template[Template$Year == i, ]$LargePredictor3/(Template[Template$Year == i, ]$LargePredictor3 + Template[Template$Year == i+1, ]$LargePredictor3))) * LargePredictor3_Mult)
  ...
  # If the year is odd
    ...
    Template[Template$Year == i, ]$Large <-
      (
          ...
          (Template[Template$Year == i, ]$TotalJobCost_Large * (Template[Template$Year == i, ]$LargePredictor3/(Template[Template$Year == i, ]$LargePredictor3 + Template[Template$Year == i-1, ]$LargePredictor3)) * LargePredictor3_Mult)
      )
...
Template %>%
  ...
  scale_x_continuous(breaks = c(seq(1994, 2009, 1))) +
  geom_col(color = NA) +
...
Template <- Template %>%
  filter(Year < 2010)
write.xlsx(Template, "C:/Users/ikennedy/JBREC/BP research public use microdata coding and data - General/AHSProject/AHSSummaries/OneYearApprox_1994_2009.xlsx")

Template_2010_2021 <- read.xlsx("C:/Users/ikennedy/JBREC/BP research public use microdata coding and data - General/AHSProject/AHSSummaries/OneYearApprox_2010_Current.xlsx")
Template_2010_2021 <- Template_2010_2021 %>%
  select(Year, CostClass, TotalJobCost_New) %>%
  filter(Year >= 2010)
FinalTemplate <- Template %>%
  rbind(Template_2010_2021) %>%
  arrange(Year)

FinalTemplate1 <- FinalTemplate %>%
  group_by(Year) %>%
  summarize(TotalJobCost = sum(TotalJobCost_New))
...
FinalTemplate %>%
  ggplot(aes(x = Year, y = TotalJobCost_New/1000000000)) +
  scale_y_continuous(labels = function(income) format(income, big.mark = ",", scientific = FALSE), breaks = c(seq(0,600,50)), limits = c(0,200)) +
  # percent_format()) +
  # function(income) format(income, big.mark = ",", scientific = FALSE)) +
  scale_x_continuous(breaks = c(seq(1994, 2021, 1))) +
  geom_line() +
  labs(
    x = "Year",
    y = "Total Spend",
    title = "Total Repair/Remodel Spend",
    caption = "Source: John Burns Research and Consulting tabulations of American Housing Survey.",
    subtitle = 'Billions of USD (Real)'
  ) +
  theme_jbrec() +
  theme(axis.text.x = element_text(angle = 90)) +
  facet_wrap(~CostClass, scales = 'free')


FinalTemplate %>%
  ggplot(aes(x = Year, y = TotalJobCost_New/1000000000, fill = fct_rev(CostClass))) +
  scale_y_continuous(labels = function(income) format(income, big.mark = ",", scientific = FALSE), breaks = c(seq(0,600,50)), limits = c(0,600), expand = c(0,1)) +
  # percent_format()) +
  # function(income) format(income, big.mark = ",", scientific = FALSE)) +
  scale_x_continuous(breaks = c(seq(1994, 2021, 1))) +
  geom_col(color = NA) +
...
write.xlsx(FinalTemplate, "C:/Users/ikennedy/JBREC/BP research public use microdata coding and data - General/AHSProject/AHSSummaries/OneYearApprox.xlsx")
